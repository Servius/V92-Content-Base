...
function EFFECT:Init(data)
	...
	if(self.Mat != nil)then
		if	mats[self.Mat][2]==1 then	self:Dust()		end
		if	mats[self.Mat][2]==2 then	self:Dirt()		end
		if	mats[self.Mat][2]==3 then	self:Sand()		end
		if	mats[self.Mat][2]==4 then	self:Metal()	end
		if	mats[self.Mat][2]==5 then	self:Smoke()	end
		if	mats[self.Mat][2]==6 then	self:Wood()		end
		if	mats[self.Mat][2]==7 then	self:Glass()	end
		if	mats[self.Mat][2]==8 then	self:Blood()	end
		if	mats[self.Mat][2]==9 then	self:YellowBlood()	end
	else 					
		--self:Smoke()
		self:Dirt()
	end
