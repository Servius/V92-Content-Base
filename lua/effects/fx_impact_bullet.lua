...
--	mats = [ material type ] = { sound, impact effect }
--	Sounds
--	1		2			3		4		5
...
local sounds = {
	[ 1 ] = { "Bullet.Dirt" } ,
	[ 2 ] = { "HL2.Physics.Concrete.Impact.Bullet" } ,
	[ 3 ] = { "HL2.Physics.Metal.Solid.Impact.Bullet" } ,
	[ 4 ] = { "HL2.Physics.Glass.Impact.Bullet" } ,
	[ 5 ] = { "HL2.Physics.Flesh.Impact.Bullet" } ,
	[ 6 ] = { "HL2.Ambient.Energy.Zap" } ,
	[ 7 ] = { "HL2.Physics.Wood.Solid.Impact.Bullet" } ,
	[ 8 ] = { "HL2.Physics.Tile.Impact.Bullet" } ,
	[ 9 ] = { "HL2.Physics.Underwater.Impact.Bullet" } ,
	[ 10 ] = { "HL2.Physics.Sand.Impact.Bullet" } ,
	[ 11 ] = { "HL2.Physics.Computer.Impact.Bullet" } ,
	[ 12 ] = { "HL2.Physics.Metal.Box.Impact.Bullet" } ,
	[ 13 ] = { "HL2.Physics.Plastic.Box.Impact.Bullet" } ,
}
...
function EFFECT:Init( data )
	...
	if ( self.Mat ~= nil ) then
		...
		if mats[ self.Mat ][ 2 ] == 2 then
			self:Dirt( )
		end

		if mats[ self.Mat ][ 2 ] == 3 then
			self:Sand( )
		end

		if mats[ self.Mat ][ 2 ] == 4 then
			self:Metal( )
		end

		if mats[ self.Mat ][ 2 ] == 5 then
			self:Smoke( )
		end

		if mats[ self.Mat ][ 2 ] == 6 then
			self:Wood( )
		end

		if mats[ self.Mat ][ 2 ] == 7 then
			self:Glass( )
		end

		if mats[ self.Mat ][ 2 ] == 8 then
			self:Blood( )
		end

		if mats[ self.Mat ][ 2 ] == 9 then
			self:YellowBlood( )
		end
	else
		--self:Smoke()
