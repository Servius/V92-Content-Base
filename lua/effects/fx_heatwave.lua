...
function EFFECT:Think()
	...
	local spawnpos = self.ent:LocalToWorld(self.offset+Vector(0,math.random(-self.radius,self.radius),math.random(-self.radius,self.radius)))
	local velocity = self.ent:GetForward()*self.power*-math.random(1000,2000)
