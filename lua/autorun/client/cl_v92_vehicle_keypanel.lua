...
surface.CreateFont("vnt_keypanel", {
	font = "Tahoma",
	size = 14
})
...
function panel:Init()
	...
	self.label:SetFont("vnt_keypanel")
	self.label:SetText("")
	self.label:SetSize(w/2,h)
	self.key = vgui.Create("DLabel",self)
	self.key:SetFont("vnt_keypanel")
...
function panel:setKey(k)
	self.key:SetText(vnt.key.name(k))
	self.keyNum=k
	self.oldkey=k
end
...
vnt.key.addHook("vnt_key_panel", function(key, pressed)
...
function panel:SetEdit(b)
	if b then
		...
		self.key:SetText(vnt.key.name(self.keyNum))
		current=nil
	end
	self.editing = b
end
...
vnt.hook("SpawnMenuOpen","vnt_key_panel_opensm", function()
	if IsValid(current) and current.editing then return false end
end)

vnt.hook("Initialize", "vnt_key_panel_initgm", function()
