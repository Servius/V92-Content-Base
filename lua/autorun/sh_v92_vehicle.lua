...
vnt.vehicle.getWeapon = function(n,t2)
	if !vnt.vehicle.weapons[n] then
		error("Could not find weaponPrimary \"" .. n .. "\"!", 2)
	end
	local t = table.Copy(vnt.vehicle.weapons[n])
	if t2 then
		table.Merge(t,t2)
	end
	return t
...
vnt.vehicle.weapons = {
	...
	["2A42"]={
		...
		func=function(self, t, p)
			if t.NextShoot <= CurTime() then
				if t.Ammo>0 then
					...
					local b=ents.Create("vnt_hc_hebullet")
					...
					b:Spawn()
					b.Owner=p
					self:EmitSound("v92/cannon/havoc_cannon_1p.wav")
					self:EmitSound("v92/cannon/havoc_cannon_3p.wav")
		...
		CalcView=function(self,t,p,pos,ang,view)
			...
			local e=self:GetNWEntity("vnt_air_radar")
			if IsValid(e) then
		...
		DrawCrosshair=function(self,t,p)
			...
			local w = sh / 6
			local s = sh / 3
			local k = p:GetNWInt("vnt_passenger_id")
			...
			local activeWeaponOne = self:GetNWInt( "seat_" .. k .. "_activePrimaryWeapon" )
			local lastShot = self:GetNWFloat( "seat_" .. k .. "_" .. activeWeaponOne .. "_lastshot" )
			local nexts = self:GetNWFloat( "seat_" .. k .. "_" .. activeWeaponOne .. "_nextshot" )
			local ammoOne = self:GetNWInt( "seat_" .. k .. "_" .. activeWeaponOne .. "_ammo" )
			
			if ammoOne == t.MaxAmmoOne and nextShotTwo > CurTime() then
			...
			local activeWeaponTwo = self:GetNWInt("seat_"..k.."_activeSecondaryWeapon")
			local lastShotTwo = self:GetNWFloat("seat_"..k.."_"..activeWeaponTwo.."_lastshot")
			local nextShotTwo = self:GetNWFloat("seat_"..k.."_"..activeWeaponTwo.."_nextshot")
			local ammoTwo = self:GetNWInt("seat_"..k.."_"..activeWeaponTwo.."_ammo")
			
			if ammoTwo == t.MaxAmmoTwo and nextShotTwo > CurTime() then
	...
	["25MM HE"]={
		...
		DrawCrosshair=function(self,t,p)
			...
			local s=sh/3
			local k=p:GetNWInt("vnt_passenger_id")
			local aw=self:GetNWInt("seat_"..k.."_activePrimaryWeapon")
			local lastShot=self:GetNWFloat("seat_"..k.."_"..aw.."_lastshot")
			local nexts=self:GetNWFloat("seat_"..k.."_"..aw.."_nextshot")
			local ammoOne=self:GetNWInt("seat_"..k.."_"..aw.."_ammo")
			local width=t.CrosshairWidth or 30
			local height=t.CrosshairHeight or 20
			local lw=t.CrosshairLinew or 30
			local lh=t.CrosshairLineh or 20
			...
			if ammoOne==t.MaxAmmoOne and nexts>CurTime() then
				surface.SetDrawColor(255,255,255,math.sin(CurTime()*10)*75+75)
			else
				surface.SetDrawColor(255,255,255,150)
			end
	...
	["M197"]={
		...
		Init=function(self,t)
			self.Sound.GunSound1=CreateSound(self,"v92/cannon/viper_cannon_1p.wav")
			self.Sound.GunSound2=CreateSound(self,"v92/cannon/viper_cannon_3p.wav")
			self.Sound.GunSoundSpin=CreateSound(self,"v92/cannon/viper_cannon_rotate.wav")
		...
		func=function(self,t,p)
			if t.NextShoot<=CurTime() then
				if t.Ammo>0 then
					...
					local rocket=ents.Create("vnt_hc_rocket")
					...
					t.rocket=rocket
					p:SetNWEntity("vnt_air_cam",rocket)
